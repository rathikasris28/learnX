...
import {
allowCodeRequest,
allowVerificationAttempt
} from '../services/otpRateLimitService.js';
...
learningLevel:
  user.learning_level || undefined,

teachingLevel:
  user.teaching_level || undefined,

learningGoal:
  user.learning_goal || undefined,
...
[input.email]
    ...
    const result =
      await client.query<UserRow>(
        ...
        [
          ...
          input.city ?? null,
          input.languages,
          input.learningGoal ?? null,
          input.learningLevel ?? null,
          input.teachingLevel ?? null,
          input.teachingStyles,
          input.availability,
          input.termsVersion,
          starterCredits
        ]
    ...
    const skillNames =
      [
        ...new Set([
          ...input.skillsLearning,
          ...input.skillsTeaching
        ])
      ];
...
* Send OTP for newly registered users.
  */
  await issueVerificationCode(
  user.id,
  user.email
